...
void BTree<T>::deleteData(Position& r, const T& e){
    ...
    if(r->getData() == e){ // Eliminación
        ...
        if(r->getLeft() != nullptr && r->getRight() != nullptr){ //Tiene los dos hijos
    ...
    if(e < r->getData()) this->deleteData(r->getLeft(), e);
    else this->deleteData(r->getRight(), e);
...
template <class T>
unsigned int BTree<T>::getHeight() const{
