...
class RepeatedIngredient : public std::runtime_error {
 public:
  explicit RepeatedIngredient(
      const std::string& msg = "El ingrediente ya esta registrado en la lista.")
      : runtime_error(msg) {}
};
...
namespace NodeExceptions{
    class NodeExceptionTo : std::runtime_error{
        public:
            explicit NodeExceptionTo( const std::string& msg = "Excepcion del Nodo") : runtime_error(msg){}
    };
} //namespace NodeExceptions

namespace TreeExceptions{
    class TreeExceptions : std::runtime_error{
        public:
            explicit TreeExceptions(const std::string& msg = "fdas") : runtime_error(msg){}
    };
}
