...
void AVLTree<T>::doBalancing(Position& r){
    ...
    switch(this->getBalanceFactor(r)){
        case -2: //Desbalanceado a la Izquierda: Rotación a la Derecha.
            if(this->getBalanceFactor(r->getLeft()) == 1) //Simple
                this->leftRotation(r->getLeft());
            this->rightRotation(r);

            break;
        case 2: //Desbalanceo a la Derecha: Rotación a la Izquierda
            if(this->getBalanceFactor(r->getRight()) == -1) //Simple
