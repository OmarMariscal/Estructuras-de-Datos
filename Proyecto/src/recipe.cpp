...
Recipe::Recipe()
    : id(-1),
      ...
      category(DESAYUNO),
      preparationTime(-1),
      procedureList(*new List<StringWrapper, Configure::maximunIngredientSize>),
...
Recipe::Recipe(const int& i,
               ...
               const Category& c,
               const int& pT,
               const List<StringWrapper, Configure::maximunIngredientSize>& pL,
...
std::string Recipe::toString(const std::string& format) const {
  ...
  if (format == "full") {
    ...
    oss << std::left << std::setw(5) << id << " | " << std::setw(22)
        << truncName << " | " << std::setw(12) << category << " | "
        << std::setw(18) << truncAuthor << " | " << std::right << std::setw(4)
        << preparationTime << " min | " << std::left << std::setw(6)
...
void Recipe::setIngredientList(
    const List<Ingredient, Configure::maximunIngredientSize>& ingredientList) {
