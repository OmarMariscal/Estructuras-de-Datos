...
void IngredientMenu::addIngredients() {
  ...
  try {
    do {
      ...
      if (!this->ingredientList.isEmpty()) {
        oss << this->centerText(
            ...
            this->insertColorText(
                "Lista de Ingredientes Actual para " + recipeName, "magenta"));
      ...
      oss << this->divider("-");
      oss << "✅ ";
      oss << this->insertColorText("Ingrediente Añadido con Exito!", "green")
          << endl;
  ...
  try {
    do {
      ...
      if (dataInt == -1) {
        ...
        oss << this->insertColorText("    [C] ", "cyan") << "Unidad de Medida"
            << endl;
        ...
        switch (atributeModify) {
          ...
          case 'C':
            oss << this->insertColorText(
                "    Ingrese la Nueva Unidad de Medida Ingrediente: ", "cyan");
            dataString = this->readLinePrompt(oss.str());
...
IngredientMenu::IngredientMenu()
    : ingredientList(*new List<Ingredient, Configure::maximunIngredientSize>),
      recipeName("default") {}

IngredientMenu::IngredientMenu(
    List<Ingredient, Configure::maximunIngredientSize>& l,
    const std::string& r)
...
void IngredientMenu::mainMenu() {
  ...
  do {
    ...
    oss << this->divider("-");
    oss << this->insertColorText("    [A] ", "cyan") << "Agregar Ingredientes"
        << endl;
    oss << this->insertColorText("    [B] ", "cyan") << "Eliminar Ingredientes"
        << endl;
    oss << this->insertColorText("    [C] ", "cyan") << "Modificar Ingredientes"
        << endl;
