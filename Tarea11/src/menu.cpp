...
void Menu::deleteAllSongs() {
  ...
  oss << "Esta seguro que desea eliminar las "
      << this->songList.getTotalElements() << " canciones? (1. Si/ 2. No): ";
...
void Menu::searchByIntepreter() {
  ...
  do {
    ...
    string songsResults;
    songsResults =
        this->songList.toString(searchedSong, Song::compareByInterpreter);
...
void Menu::sortBySongName() {
  ...
  oss << "[D] Ordenamiento por ShellSort (Ordenamiento Sumamente Ineficiente "
