...
void Matrix::randomizer() {
  ...
  std::default_random_engine engine(
      seed());  // Usamos el motor aleatorio por defecto, el más simple, con la
                // semilla generada
  std::uniform_real_distribution<float> distribution(
      -100.00, 100.00);  // Crear el objeto de una distribución normal de reales
...
Matrix Matrix::operator*(const Matrix& otro) const {
  Matrix times(size);
  // Recorrer ambas matrices en un determinado orden siguiendo el algoritmo de
  // multiplicación.
  for (int i = 0; i < size; i++)
    for (int j = 0; j < size; j++)
