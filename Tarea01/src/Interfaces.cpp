...
void Menu::mainMenu() {
  ...
  // El menú principal es un string acompañado de ostringstream para poder
  ...
  switch (selection) {
    ...
    case 3:
      exit();
      break;
      // No hay necesidad de un case default; esto por las utilidades que no
...
void Menu::dataTypes() {
  ...
  cout << "La siguiente tabla muestra la informacion de distintos tipos de "
          "datos obtenidos directamente del lenguaje: "
       << endl;
  Tables::DataTypeTable::showDataTypeTable();  // Uso del método estático (sin
                                               // instancia requerida)
