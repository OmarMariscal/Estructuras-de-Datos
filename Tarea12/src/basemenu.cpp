...
int BaseMenu::readPositiveInteger(std::string& prompt,
...
float BaseMenu::readPositiveFloat(const std::string& prompt,
                                  const bool& ceroAllowed) {
  std::string aux("");
  float result;
  while (true) {
  ...
  for (size_t i = 0; i < s.size(); ++i) {
    ...
    if (c == 0x1B) {  // ESC
      ...
      if (i + 1 < s.size() && s[i + 1] == '[') {
        ...
        // por eso dejamos que continúe normalmente (se descarta la letra final
        // también)
        continue;
      } else {
...
#else
  system("clear");  // Para Linux, macOS y otros sistemas
#endif
}
