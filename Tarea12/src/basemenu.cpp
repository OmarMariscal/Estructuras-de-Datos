...
int BaseMenu::readPositiveInteger(std::string& prompt, const bool& ceroAllowed){
  string aux("");
  int result;
  while (true) {
    try {
...
float BaseMenu::readPositiveFloat(const std::string& prompt, const bool& ceroAllowed){
  std::string aux("");
  float result;
  while (true) {
    try {
