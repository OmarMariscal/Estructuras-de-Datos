...
void Menu::makeTranformation() {
  ...
  do {  // Bucle para repetir transformaciones
    ...
    while (true) {
      try {
        ...
        oss << this->insertColorText(
            "    Ingrese la operacion en notacion infija: ", "cyan");
        ...
        this->errorMessage(
            "La expresion no es valida.\nRecuerde que no se pueden tener dos "
            "operadores juntos (3++5)\nParentesis que no se cierran. \nNo se "
            "puede empezar con ^ entre otras cosas \n(Si puede poner "
            "espacios)");
    ...
    oss << this->insertColorText(" ✅ La conversion es: ", "green")
        << this->transformer.postFixToString() << endl;
    // Agregarla al Historial
    this->record.insertData(
        this->record.getLastPos(),
        infixNotation + "   ->   " + this->transformer.postFixToString());
    oss << this->insertColorText(" 📈 Conversion agregada al historial.",
                                 "yellow")
        << endl
        << endl;
    oss << this->divider("-");

    oss << this->insertColorText(" 🔁 ¿Desea Realizar Otra Conversión? (S/N): ",
                                 "magenta");
...
void Menu::showRecord() {
  if (this->record.isEmpty()) {
    this->errorMessage(
        "No Existen Transformaciones Realizadas\nRealice alguna antes de "
        "entrar aquí.");
    return;
  }
  ...
  cout << this->windowHeader(this->insertColorText(" 📈 Historial de Tranformaciones", "green"));
  cout <<this->insertColorText(this->centerText(this->record.toString()),
                                "cyan");
...
void Menu::deleteRecords() {
  ...
  this->centerText(this->insertColorText(
      "Actualmente Hay Un Total de: " +
          to_string(this->record.getTotalElements()) + " Registros",
      "cyan"));
  this->divider("-");
  oss << this->insertColorText(
      " ❌ Esta seguro que desea eliminar el historial de transformaciones? "
      "(S/N): ",
      "red");
  ...
  if (response == 'S') {
    ...
    oss << this->centerText(this->insertColorText(
               "Historial Eliminado Con Exito!", "yellow"))
        << endl;
  } else {
    oss << this->centerText(
               this->insertColorText("Operación Cancelada.", "yellow"))
        << endl;
  }
...
void Menu::exitScreen() {
  ...
  cout << this->insertColorText("    Programa Hecho Por: ", "cyan")
       << "Mariscal Rodríguez Omar Jesús" << endl;
  cout << this->insertColorText("    Profesor: ", "cyan")
       << "Gutiérrez Hernández Alfredo" << endl;
  cout << this->divider("-");
...
Menu::Menu(NotationTransformer& t, List<StringWrapper>& r)
...
void Menu::mainMenu() {
  ...
  bool op = true;
  oss << this->windowHeader(this->insertColorText(
      "   ✏️  Tranformador de Notacion Infija a Postfija", "green"));
  oss << this->centerText("Por favor, seleccione una opción: ");
  oss << this->divider("-");
  oss << this->insertColorText("    [T]", colorChars)
      << this->insertColorText(" 🧮 Transformar una Notación", colorOps)
      << endl;
  oss << this->insertColorText("    [R]", colorChars)
      << this->insertColorText(" 📈 Revisar el Historial de Transformaciones",
                               colorOps)
      << endl;
  oss << this->insertColorText("    [E]", colorChars)
      << this->insertColorText(" ❌ Eliminar el Historial de Transformaciones",
                               colorOps)
      << endl;
  oss << this->insertColorText("    [S]", colorChars)
      << this->insertColorText(" 👋 Salir del Programa", colorOps) << endl;
  oss << this->divider("-");
