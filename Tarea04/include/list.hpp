...
std::string List<T, ARRAYSIZE>::toString() const {
  ...
  int widthBorder = (widthRanking * 2) + widthSongName + (widthName * 2) + 1;
  ...
  oss << std::left << std::setw(widthSongName) << "| Nombre de la Cancion";
  ...
  for (int i = 0; i <= last; i++) {
    oss << "| " << std::left << std::setw(widthRanking - 2) << i << "| "
        << std::left << std::setw(widthRanking - 2) << data[i].getRanking()
