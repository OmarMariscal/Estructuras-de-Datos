...
void Menu::searchBySongName() {
    ...
    do {
        ...
        List<Song>::Position currentPos = this->songList.getFirstPos();

        while (currentPos != nullptr) {
            Song& currentSong = this->songList.retrieve(currentPos);
            if (Song::compareBySongName(currentSong, searchedSong) == 0) {
                songWithTheName.insertData(currentSong,0);
            }
            currentPos = this->songList.getNextPos(currentPos);
        }
        ...
        if (songWithTheName.isEmpty()) {
            oss << "\nNo existe un registro de una cancion llamada: " << songName << endl;
        } else {
...
void Menu::searchByIntepreter() {
    ...
    do {
        ...
        while (currentPos != nullptr) {
            Song& currentSong = this->songList.retrieve(currentPos);
            if (Song::compareByInterpreter(currentSong, searchedSong) == 0) {
                songsOfInterpreter.insertData(currentSong,0);
            }
            currentPos = this->songList.getNextPos(currentPos);
        }
